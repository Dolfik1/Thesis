...
class SGUCell(RNNCell):
  """
    SGU as described in http://proceedings.mlr.press/v63/gao30.pdf
  """
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with _checked_scope(self, scope or "sgu_cell", reuse=self._reuse):
      with vs.variable_scope("xg"):  # Reset gate and update gate.
        # We start with bias of 1.0 to not reset and not update.
        xg = _linear(inputs, self._num_units, True, 1.0)
      with vs.variable_scope("zg"):
        zg = sigmoid(_linear(state * xg,self._num_units, False, 1.0))
        z_out = sigmoid(zg * state)

      with vs.variable_scope("zt"):
        zt = sigmoid(_linear([inputs,state], self._num_units, True, 1.0))

      new_h = (1 - zt) * state + zt * z_out
    return new_h, new_h
